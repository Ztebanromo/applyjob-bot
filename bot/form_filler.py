...
def _generate_ai_answer(question: str, profile: dict) -> str:
    """Genera una respuesta inteligente basada en la pregunta y el perfil."""
    q = question.lower()
    
    # Lógica de deducción simple (preparada para LLM en el futuro)
    if any(k in q for k in ["por qué", "why", "interés", "interest", "motivation"]):
        return f"Me motiva este puesto porque mi experiencia previa se alinea con los desafíos del rol y busco aportar valor desde el primer día."
    
    if any(k in q for k in ["años", "years", "experience", "experiencia"]):
        return str(profile.get("years_exp", "5"))
        
    if any(k in q for k in ["salario", "salary", "pretension", "expectativa"]):
        return str(profile.get("salary", "A convenir"))

    # Si es una pregunta de texto largo genérica, enviar el resumen de experiencia
    return profile.get("experience_summary", "Cuento con sólida formación técnica y experiencia en proyectos similares.")
    ...
    for sel in selectors:
        ...
        for el in elements:
            try:
                ...
                elif sel == "textarea" or "question" in attrs.lower():
                    # Si es un campo de texto largo sin match, usar IA
                    value = _generate_ai_answer(attrs, profile)
