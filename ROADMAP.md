...
- [x] `dashboard/app.py` — Servidor FastAPI independiente
- [x] Tabla de postulaciones con filtros y estados
- [x] Diseño Premium Glassmorphism (Modo Oscuro)
- [x] Gráficos estadísticos con Chart.js
- [x] `python dashboard/app.py` → `http://localhost:8000`
...
**Objetivo:** setup inicial sin tocar código (Integrado en Dashboard).

- [x] Refactorización de `bot/config.py` para carga dinámica
- [x] Formulario web en Dashboard para editar `USER_PROFILE`
- [x] Edición de parámetros de búsqueda (LinkedIn URL) desde la web
- [x] Persistencia en `data/user_config.json`
...
+ **Arquitectura Independiente**: Cada portal ahora tiene su propio manejador.

**Objetivo:** flujo nativo de Indeed Easy Apply.

- [x] `bot/portals/indeed.py` — `IndeedPortal`
- [x] Detección de Indeed Apply vs external redirect
- [x] Modal multi-step de Indeed (vía iframe/página)
- [x] Registro en portal registry
- [x] Refactorización del `engine.py` para usar handlers unificados

**Esfuerzo:** ~4h | **Riesgo:** alto (completado)
