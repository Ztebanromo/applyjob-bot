...
from bot.chrome_cookie_importer import (
    import_portal_cookies   as _import_portal_cookies,
    chrome_cookies_available as _chrome_cookies_available,
)
...
# ── CDP (Chrome real del usuario) ─────────────────────────────────────────────
from bot.chrome_cdp import (
    connect       as _cdp_connect,
    disconnect    as _cdp_disconnect,
    get_status    as _cdp_get_status,
    check_session_cdp,
    is_port_open  as _cdp_port_open,
)


@app.route('/api/cdp-status', methods=['GET'])
def api_cdp_status():
    """Estado actual de la conexión CDP con Chrome real."""
    return jsonify(_cdp_get_status())


@app.route('/api/cdp-connect', methods=['POST'])
def api_cdp_connect():
    """
    Intenta conectar al Chrome con debug habilitado.
    Primero verifica que el puerto 9222 esté abierto.
    """
    ...
    ok = _cdp_connect()
    status = _cdp_get_status()
    if ok:
        socketio.emit('cdp_status', status, namespace='/bot')
    return jsonify({'ok': ok, **status})


@app.route('/api/cdp-disconnect', methods=['POST'])
def api_cdp_disconnect():
...
@app.route('/api/cdp-verify-sessions', methods=['POST'])
def api_cdp_verify_sessions():
    """
    Verifica sesiones usando el Chrome real (más confiable que headless).
    Body JSON: { "portals": ["linkedin", "laborum"] }
    """
    if not _cdp_port_open():
        return jsonify({'error': 'Chrome no está en modo debug'}), 503

    if not _cdp_connect():
        return jsonify({'error': 'No se pudo conectar al Chrome'}), 503

    requested = (request.json or {}).get('portals') or list(_PORTALS_REQUIRE_LOGIN)
    portals   = [p for p in requested if p in _PORTALS_REQUIRE_LOGIN]

    results = {}
    for portal in portals:
        socketio.emit('session_check_progress', {portal: 'checking'}, namespace='/bot')
        result = check_session_cdp(portal)
        results[portal] = result
        socketio.emit('session_check_progress', {portal: result}, namespace='/bot')
...
def api_check_sessions():
    ...
    try:
        ...
        # Si CDP disponible → verificar todos los portales via Chrome real (más rápido y confiable)
        from bot.chrome_cdp import is_port_open as _cdp_available, connect as _cdp_conn
        if _cdp_available() and _cdp_conn():
            _log.info("[CHECK_SESSIONS] CDP activo — verificando via Chrome real")
            for portal in portals_to_check:
                socketio.emit('session_check_progress', {portal: 'checking'}, namespace='/bot')
                r = check_session_cdp(portal)
                results[portal] = r
                socketio.emit('session_check_progress', {portal: r}, namespace='/bot')
            socketio.emit('session_status', get_session_status(), namespace='/bot')
            return jsonify(results)

        # Sin CDP → chequeo rápido por cookies, luego headless
        for portal in portals_to_check:
...
def api_import_chrome_cookies():
    """
    Importa cookies del Chrome real del usuario al perfil del bot.
    Útil cuando el login en el browser de Playwright falla (especialmente LinkedIn).

    Body JSON: { "portals": ["linkedin", "laborum"] }  — vacío = todos los que requieren login.
    Response: { portal: "imported" | "no_cookies" | "chrome_unavailable" | "error" }
    """
    if not _chrome_cookies_available():
        return jsonify({"error": "Chrome no encontrado en esta máquina"}), 404

    requested = (request.json or {}).get("portals") or list(_PORTALS_REQUIRE_LOGIN)
    portals_to_import = [p for p in requested if p in _PORTALS_REQUIRE_LOGIN]

    results = {}
    for portal in portals_to_import:
        session_dir = os.path.join(_SESSIONS_DIR, portal)
        try:
            ok = _import_portal_cookies(portal, session_dir)
            results[portal] = "imported" if ok else "no_cookies"
        except Exception as exc:
            _log.warning("[IMPORT_CHROME] Error en %s: %s", portal, exc)
            results[portal] = "error"
